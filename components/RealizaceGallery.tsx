...
import { MapPin, ZoomIn, X, ArrowRight } from "lucide-react";
  ...
  return (
    <section className="py-20 bg-white text-slate-900 relative" id="realizace">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {displayedItems.map((item) => (
            <div
              ...
              className="group relative aspect-square rounded-3xl overflow-hidden bg-slate-100 border border-slate-200 hover:border-red-500 cursor-pointer transition-all duration-300 shadow-sm hover:shadow-xl"
            >
              <Image
                src={item.image}
                alt={item.title}
              ...
              <div className="absolute top-3 left-3 right-3 flex items-center justify-between pointer-events-none">
                <span className="px-2.5 py-1 rounded-lg bg-white/90 backdrop-blur-md border border-slate-200 text-[10px] font-bold text-slate-800 uppercase tracking-wider shadow-sm">
                  {item.categoryLabel}
                </span>
              ...
              <div className="absolute bottom-4 left-4 right-4 space-y-1">
                ...
                <div className="pt-2 flex items-center gap-1 text-[11px] font-semibold text-red-300 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        {showViewAll && (
          <div className="mt-12 text-center">
            <Link
              href="/realizace"
              className="inline-flex items-center gap-2 px-8 py-3.5 bg-slate-900 hover:bg-red-600 text-white font-bold text-sm transition-all rounded-2xl shadow-md hover:shadow-lg"
            >
      ...
      {selectedItem && (
        ...
        >
          <div
            className="relative max-w-4xl w-full bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Close Button */}
            <button
              onClick={() => setSelectedItem(null)}
              className="absolute top-4 right-4 z-10 p-2.5 rounded-full bg-black/60 hover:bg-red-600 text-white transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="p-6 bg-white border-t border-slate-200 space-y-2">
              <div className="flex items-center justify-between">
                <span className="px-3 py-1 bg-red-100 border border-red-200 text-red-700 text-xs font-bold rounded-lg uppercase">
              ...
              <p className="text-xs sm:text-sm text-slate-600">{selectedItem.description}</p>
            </div>
          </div>
        </div>
      )}
